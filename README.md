...
```
 ...
 sl_instance.addCallback(callback); - добавляет функцию обратного вызова для получения данных из слайдера при изменениях
 Пример callback:
 const callback = {
  handleEvent: (result) => {
    console.log(result);
 ...
 sl_instance.removeCallback(callback); - отменяет подписку на изменения слайдера для данной функции обратного вызова
 ```
## Описание проекта:
### Основные классы:
